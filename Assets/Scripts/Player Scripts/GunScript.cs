...
public class GunScript : MonoBehaviour
{
	...
	*/
    ...
    {// Running();  so i can find it with CTRL + F
        if (Input.GetAxis("Vertical") > 0 && Input.GetAxisRaw("Fire2") == 0 && meleeAttack == false && Input.GetAxisRaw("Fire1") == 0)
        {
    ...
    public bool aiming;
    /*
	 * Checking if meleeAttack is already running.
	 * If we are not reloading we can trigger the MeleeAttack animation from the IENumerator.
	 */
    void MeleeAnimationsStates()
    {
        if (handsAnimator)
        {
            meleeAttack = handsAnimator.GetCurrentAnimatorStateInfo(0).IsName(meeleAnimationName);
	...
	*/
    void MeleeAttack()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Q) && !meleeAttack)
        {
	...
	*/
    ...
    {
        handsAnimator.SetBool("meleeAttack", true);
        //yield return new WaitForEndOfFrame();
        yield return new WaitForSeconds(0.1f);
        handsAnimator.SetBool("meleeAttack", false);
    }
	...
	*/
    ...
    [Tooltip("Array of muzzle flashes, randmly one will appear after each bullet.")]
    public GameObject[] muzzleFlash;
    [Tooltip("Place on the gun where muzzle flash will appear.")]
    ...
    private void ShootMethod()
    {
        if (waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
        {
            ...
            if (bulletsInTheGun > 0)
            {
                ...
                holdFlash = Instantiate(muzzleFlash[randomNumberForMuzzleFlash], muzzleSpawn.transform.position /*- muzzlePosition*/, muzzleSpawn.transform.rotation * Quaternion.Euler(0, 0, 90)) as GameObject;
	...
	*/
    ...
    IEnumerator Reload_Animation()
    {
        if (bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading/* && !aiming*/)
        {
            ...
            if (meleeAttack == false && pmS.maxSpeed != runningSpeed)
            {
                //print ("tu sam");
	...
	*/
    void Animations()
    {
        ...
        if (handsAnimator)
        {
            ...
            if (Input.GetKeyDown(KeyCode.R) && pmS.maxSpeed < 5 && !reloading && !meleeAttack/* && !aiming*/)
            {
                StartCoroutine("Reload_Animation");
            }
        }
